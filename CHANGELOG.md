...
- TLS listener 在分配 TLS 对象前限制接入速率、全局及每 IP 未认证连接数；耗尽预算时
  定时等待，同一来源重复握手失败后临时拒绝，避免扫描/洪泛拖垮服务端。
- TLS 与应用认证共用总超时期限，认证成功或连接清理后释放未认证配额。
- 聚合重复 TLS 错误日志并新增接入保护指标；增加真实 TLS/明文洪泛、慢连接、认证配额
  释放与关闭回归测试。
- 修复 Linux NAT 穿透测试的公网拓扑：将两条 NAT 上联接入同一个网桥，避免同网段
  独立接口产生错误路由和不可达的 ARP；失败时输出进程日志、路由与邻居表。
- 修复 P2P 打洞端口不匹配：客户端复用引导连接的观测端口，服务端释放直连监听端口后
  再发起打洞；为快速失败的重试增加间隔，避免 RST 触发 CPU 与 SYN 洪泛。
