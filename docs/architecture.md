...
默认路径为 `/var/lib/minitun/state.db`。版本 3 包含：
...
- `tunnels`：`id`、可空且非唯一的 `name`、`server_id`、TCP 协议、本地和远程
  主机/端口对、期望状态与实际状态、可空的最后错误码和错误消息、创建/更新时间戳，
  以及可空的最近远端同步时间戳；
...
每个在线客户端会话比较持久化隧道期望状态与内存注册集合；本地控制提交会立即唤醒
对应会话，心跳处理负责存活检测和兜底同步。缺失的活动隧道从 `registering` 转为
`active`；服务端策略或绑定失败只会使该隧道转为 `failed` 并记录稳定错误码，之后的
同步会重试。已删除或禁用的隧道会被幂等注销。会话断开时清除运行时集合，并在一个
SQLite 事务中把全部活动隧道恢复为 `pending`，使新代次能在服务端或守护进程重启后
重新创建所有监听器。
...
两秒。服务端通过兼容旧客户端的心跳序列通告 `1-300` 秒空闲期限，新客户端使用该
期限并增加 5 秒关闭宽限，避免先于服务端回收 Worker。
